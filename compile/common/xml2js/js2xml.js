...
const { isArray } = require("./array-helper");
...
function writeAttributes(attributes, options, depth) {
  ...
  for (key in attributes) {
    if (
      ...
      if (!["value"].includes(attrName) && attr === "") {
      ...
      case "element":
        if (element.elements && element.elements.length > 1) {
          for (let i = element.elements.length - 1; i > 0; i--) {
            if (element.elements[i - 1].type === "text") {
              element.elements[i].__noIndent = true;
            }
          }
        }
        return (
          xml +
          (element.__noIndent ? "" : indent) +
          writeElement(element, options, depth)
        );
      case "comment":
        return (
